...
public class SqlAnalyzer {
    ...
    public static String convertTwoWaySql2DisplaySql(SqlAnalyzerFactory factory, String twoWaySql, Object arg,
        ...
        return convertTwoWaySql2DisplaySql(
                factory,
                twoWaySql,
                argNames,
                argTypes,
    ...
    // =======
    public Node analyze() {
        push(createRootNode()); // root node of allColumns
    ...
    protected void processSqlConnectorAdjustable(Node node, String sql) {
        ...
        st.skipWhitespace();
        final String skippedToken = st.skipToken(); // determination for and/or
        // (also skip process)
        st.skipWhitespace();
    ...
    protected boolean processSqlConnectorCondition(Node node, SqlTokenizer st, String skippedToken) {
        if ("and".equalsIgnoreCase(skippedToken) || "or".equalsIgnoreCase(skippedToken)) { // is
            // connector
    ...
    protected void parseComment() {
        ...
        } else if (Srl.is_NotNull_and_NotTrimmedEmpty(comment)) { // plain
            // comment
    ...
    protected boolean isTargetComment(String comment) {
        ...
        if (!comment.startsWith(ForNode.CURRENT_VARIABLE)) { // except current
            // variable from
            // check
    ...
    protected EmbeddedVariableNode createEmbeddedVariableNode(String expr, String testValue, boolean replaceOnly,
                                                              boolean terminalDot) {
        researchIfNeeds(_researchEmbeddedVariableCommentList, expr); // for
        // research
    ...
    protected SqlConnectorNode createSqlConnectorNode(Node node, String connector, String sqlParts) {
        if (isNestedBegin(node)) { // basically nested if BEGIN node because
            // checked before
     ...
     */
    public List<String> researchIfComment() { // should NOT be called with
        // execution
    ...
    /**
     * Research bind variable comments. (basically for research only, NOT for
     * execution)<br />
     * This method should be called before calling analyze(). <br />
     * The returned list is filled with bind variable comment after calling
     * analyze().
     *
     * @return The list of bind variable comment. (NotNull)
     */
    public List<String> researchBindVariableComment() { // should NOT be called
        // with execution
        final List<String> resultList = new ArrayList<String>();
    ...
    /**
     * Research embedded variable comments. (basically for research only, NOT
     * for execution)<br />
     * This method should be called before calling analyze(). <br />
     * The returned list is filled with embedded variable comment after calling
     * analyze().
     *
     * @return The list of embedded variable comment. (NotNull)
     */
    public List<String> researchEmbeddedVariableComment() { // should NOT be
        // called with
        // execution
        final List<String> resultList = new ArrayList<String>();
    ...
    protected String ln() {
        return "\n"; // LF is basic here
        // /- - - - - - - - - - - - - - - - - - - - - - - - - - -
        // The 'CR + LF' causes many trouble allColumns over the world.
